...
def _apply_runtime_api_keys(job_data: dict) -> dict:
    """Apply resolved API keys from job payload to current process env and config."""
    resolved = job_data.get("resolved_api_keys", {}) if isinstance(job_data, dict) else {}
    payload_sources = job_data.get("resolved_api_key_sources", {}) if isinstance(job_data, dict) else {}
    source_maps = [
        ("job_payload", dict(resolved or {})),
        (
            "process_env",
            {key: os.environ.get(key, "") for key in KEY_NAMES},
        ),
    ]
    chosen, sources = resolve_api_keys(source_maps)

    try:
        import config as _config
        from config import LLM_CONFIG
        apply_resolved_api_keys(chosen, sources, config_module=_config, llm_config=LLM_CONFIG)
    except Exception as exc:
        logger.warning("[WORKER-KEYS] Failed to patch runtime keys into config: %s", exc)

    openai_src = payload_sources.get("OPENAI_API_KEY", sources.get("OPENAI_API_KEY", "missing"))
    serper_src = payload_sources.get("SERPER_API_KEY", sources.get("SERPER_API_KEY", "missing"))
    logger.info(
        "[WORKER-KEYS] OPENAI=%s via %s | SERPER=%s via %s",
        "set" if is_real_api_key(os.environ.get("OPENAI_API_KEY", ""), "OPENAI_API_KEY") else "missing",
        openai_src,
        "set" if is_real_api_key(os.environ.get("SERPER_API_KEY", ""), "SERPER_API_KEY") else "missing",
        serper_src,
    )
    return chosen
...
def process_single_keyword(kw, idx, total, config_opts, output_dir, glog, csv_log):
    ...
    gsheet_row = -1
    try:
        build_version = str(config_opts.get("build_version", "") or "").strip() or "unknown"
        # 1. Tạo dòng trong CSV (luôn thành công)
        csv_row = csv_log.start_keyword(kw)
        logger.info("Worker [%d/%d]: Bắt đầu '%s' (CSV row=%d, version=%s)", idx, total, kw, csv_row, build_version)

        # 2. Import pipeline function (lazy import để tránh circular)
        if glog and glog.is_connected:
            gsheet_row = glog.start_keyword(kw)
        ...
        try:
            if glog and glog.is_connected and gsheet_row > 0:
                ...
                glog.log_error(gsheet_row, error_msg)
                print("✅ [GSHEET] Đã ghi lỗi lên Sheet.")
            else:
                print(f"⚠️ [GSHEET] Bỏ qua ghi lỗi (csv_row={csv_row}, connected={glog.is_connected if glog else None}).")
...
def main():
    ...
    _apply_runtime_api_keys(job_data)

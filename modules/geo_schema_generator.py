...
def generate_geo_schemas(brief: Dict, project=None) -> str:
    ...
    # Fix: use isinstance check instead of "or" (empty list is falsy but valid data)
    paa_from_serp = brief.get("serp_analysis", {})
    if isinstance(paa_from_serp, dict):
        paa_from_serp = paa_from_serp.get("people_also_ask", [])
    elif not isinstance(paa_from_serp, list):
        paa_from_serp = []
    paa_from_analysis = brief.get("suggested_questions", [])
    if not isinstance(paa_from_analysis, list):
        paa_from_analysis = []
    paa_all = (paa_from_serp if isinstance(paa_from_serp, list) else []) + \
               (paa_from_analysis if isinstance(paa_from_analysis, list) else [])
    paa_real = [q for q in paa_all if q and not str(q).startswith("N/A")]
    micro_data = []

    if paa_real:
        faq_entities = []
        micro_data_raw = brief.get("micro_briefing", [])
        # Fix: normalize dict wrapper (same as koray_analyzer)
        if isinstance(micro_data_raw, dict):
            micro_data = micro_data_raw.get("items", []) if "items" in micro_data_raw else list(micro_data_raw.values())
        elif isinstance(micro_data_raw, list):
            micro_data = micro_data_raw
        else:
            micro_data = []
    ...
    # Fix: search_intent can be dict {"type": "..."} or string
    search_intent_raw = brief.get("search_intent", "")
    if isinstance(search_intent_raw, dict):
        search_intent = str(search_intent_raw.get("type", "")).lower()
    else:
        search_intent = str(search_intent_raw or "").lower()
    ...
    # Check opening sentence — reuse micro_data normalized above
    sapo = ""
    if micro_data:
        sapo = str(micro_data[0].get("snippet", ""))
